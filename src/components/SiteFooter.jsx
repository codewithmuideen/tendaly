...
const SiteFooter = () => {
  ...
  // Data updated to match the image structure
  const companyLinks = [
        ...
        <div className="mx-auto max-w-7xl px-6 pb-8 pt-16 sm:pt-24 lg:px-8 lg:pt-32">
          <div className="xl:grid xl:grid-cols-4 xl:gap-8">
            ...
            <div className="space-y-8">
              <img className="h-10 w-auto" src={images.logo3} alt="Tendaly" />
              <p className="text-gray-300">
                Join families who trust Tendaly to care for their loved ones at home. Sign up
                and get notified when the app is ready.
              </p>

              {/* Store buttons: Updated styles to match image */}
              <div className="mt-6 flex flex-wrap gap-3">
                {/* Apple Store */}
                <button
                  onClick={() => setShowModal(true)}
                  className="flex items-center bg-black text-white px-4 py-2.5 rounded-lg
                ...
                <button
                  onClick={() => setShowModal(true)}
                  className="flex items-center bg-[#0b5aad] text-white px-4 py-2.5 rounded-lg
            ...
            {/* Link Columns: Restructured for Company, Support, and Connect */}
          ...
          {/* Footer Bottom: Restructured for legal links and copyright */}
          <div className="mt-16 border-t border-white/10 pt-8 sm:mt-20 lg:mt-24 md:flex md:items-center md:justify-between">
            <div className="flex flex-wrap gap-x-6 gap-y-2 md:order-1">
              {legalLinks.map((link) => (
                <a key={link.name} href={link.href} className="text-sm leading-6 text-gray-300 hover:text-white">
                  {link.name}
                </a>
              ))}
            </div>
            <p className="mt-8 text-sm leading-6 text-gray-400 md:order-2 md:mt-0">
